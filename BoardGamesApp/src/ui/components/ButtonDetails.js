...
const ButtonDetails = ({ item, deleteOperation, navigateUrl }) => {
    ...
    const handleDeleteClick = (item) => {
        console.log(item)
        confirm({ description: `Do you want to delete the ${item.name}?`})
        .then( async () => {
            await deleteOperation(item);
    ...
    return(
        <div className="buttons d-flex justify-content-end gap-2">
            ...
            <button type="button" className="btn btn-outline-danger d-flex align-items-center gap-1" onClick={() => handleDeleteClick(item)}>
